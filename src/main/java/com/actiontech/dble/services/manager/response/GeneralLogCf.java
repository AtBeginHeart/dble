...
public final class GeneralLogCf {
                ...
                try {
                    ...
                    if (isOn) {
                        if (!GeneralLogProcessor.getInstance().isEnable())
                            GeneralLogHelper.putGLog(FILE_HEADER.toString());
    ...
    public static class ReloadGeneralLogFile {
        ...
        private static boolean isCreateFile(File file) {
            try {
                if (!file.exists()) {
